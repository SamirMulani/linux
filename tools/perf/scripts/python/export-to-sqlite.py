...
"""
    export-to-sqlite.py is used to export perf data to SQLite Database
...
def trace_end():
    ...
    if PERF_DB_EXPORT_CALLS:
        ...
        do_query(
            query, 'UPDATE comms SET has_calls = 1 WHERE comms.id IN (SELECT DISTINCT comm_id FROM calls)')
