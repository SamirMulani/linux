...
"""
    export-to-postgresql.py is used to export perf data to Postgresql Database
    All the perf data will be recorded and displayed in a structured format
...
# Older versions may not support -s, in which case answer the prompt below:
...
# $ sudo apt-get install python3-pyside6.qtsql libqt5sql5-psql
#
...
# when something (a selected event) happened.
# The memory address is the instruction pointer or 'ip'.
#
# calls
#
...
# call_paths
#
# 'call_paths' represents all the call stacks.
# Each 'call' has an associated record in 'call_paths'.
...
#
# Show function calls made by function in the same context (e.g. one with call_path_id 254):
#
# SELECT * FROM calls_view WHERE parent_call_path_id = 254;
#
# Show branches made during a function call
# (e.g. where call_id is 29357 and return_id is 29370 and tid is 29670)
#
...
def usage():
    printerr(
        "Usage is: export-to-postgresql.py <database name> [<COLUMNS>] [<calls>] [<callchains>] [<pyside-version-1>]")
...
def copy_output_file(file, table_name):
    ...
    while len(data):
        ...
        if ret != 1:
            raise Exception(
                "COPY FROM STDIN PQputCopyData failed, error " + str(ret))
        data = file.read(65536)
    ret = PQputCopyEnd(conn, None)
    if ret != 1:
        raise Exception(
            "COPY FROM STDIN PQputCopyEnd failed, error " + str(ret))
    PQfinish(conn)
...
def trace_begin():
    ...
    # id == 0 means unknown.
    # It is easier to create records for them than replace the zeroes with NULLs
...
def trace_end():
    ...
    if PERF_DB_EXPORT_CALLS:
        ...
        do_query(
            query, 'UPDATE comms SET has_calls = TRUE WHERE comms.id IN (SELECT DISTINCT comm_id FROM calls)')
...
def thread_table(thread_id, machine_id, process_id, pid, tid, *x):
    value = struct.pack("!hiqiqiqiiii", 5, 8, thread_id, 8,
                        machine_id, 8, process_id, 4, pid, 4, tid)
...
def comm_table(comm_id, comm_str, thread_id, time, exec_flag, *x):
    ...
    value = struct.pack(fmt, 5, 8, comm_id, n, comm_str, 8,
...
def dso_table(dso_id, machine_id, short_name, long_name, build_id, *x):
    ...
    value = struct.pack(fmt, 5, 8, dso_id, 8, machine_id,
                        n1, short_name, n2, long_name, n3, build_id)
...
def symbol_table(symbol_id, dso_id, sym_start, sym_end, binding, symbol_name, *x):
    ...
    value = struct.pack(fmt, 6, 8, symbol_id, 8, dso_id, 8,
                        sym_start, 8, sym_end, 4, binding, n, symbol_name)
...
def sample_table(sample_id, evsel_id, machine_id, thread_id, comm_id, dso_id,
                 symbol_id, sym_offset, ip, time, cpu, to_dso_id, to_symbol_id,
                 to_sym_offset, to_ip, period, weight, transaction, data_src,
                 branch_type, in_tx, call_path_id, insn_cnt, cyc_cnt, flags, *x):
    if branches:
        value = struct.pack("!hiqiqiqiqiqiqiqiqiqiqiiiqiqiqiqiiiBiqiqiqii", 21, 8,
                            sample_id, 8, evsel_id, 8, machine_id, 8, thread_id, 8,
                            comm_id, 8, dso_id, 8, symbol_id, 8, sym_offset, 8, ip,
                            8, time, 4, cpu, 8, to_dso_id, 8, to_symbol_id, 8,
                            to_sym_offset, 8, to_ip, 4, branch_type, 1, in_tx, 8,
                            call_path_id, 8, insn_cnt, 8, cyc_cnt, 4, flags)
    else:
        value = struct.pack("!hiqiqiqiqiqiqiqiqiqiqiiiqiqiqiqiqiqiqiqiiiBiqiqiqii",
                            25, 8, sample_id, 8, evsel_id, 8, machine_id, 8, thread_id,
                            8, comm_id, 8, dso_id, 8, symbol_id, 8, sym_offset, 8, ip,
                            8, time, 4, cpu, 8, to_dso_id, 8, to_symbol_id, 8,
                            to_sym_offset, 8, to_ip, 8, period, 8, weight, 8,
                            transaction, 8, data_src, 4, branch_type, 1, in_tx, 8,
                            call_path_id, 8, insn_cnt, 8, cyc_cnt, 4, flags)
...
def call_return_table(cr_id, thread_id, comm_id, call_path_id, call_time, return_time,
                      branch_count, call_id, return_id, parent_call_path_id, flags,
                      parent_id, insn_cnt, cyc_cnt, *x):
    fmt = "!hiqiqiqiqiqiqiqiqiqiqiiiqiqiq"
    value = struct.pack(fmt, 14, 8, cr_id, 8, thread_id, 8, comm_id, 8, call_path_id, 8, call_time,
                        8, return_time, 8, branch_count, 8, call_id, 8, return_id, 8,
                        parent_call_path_id, 4, flags, 8, parent_id, 8, insn_cnt, 8, cyc_cnt)
...
def cbr(id, raw_buf):
    ...
    value = struct.pack("!hiqiiiiii", 4, 8, id, 4, cbr,
                        4, int(MHz), 4, int(percent))
...
def pwrx(id, raw_buf):
    ...
    value = struct.pack("!hiqiiiiii", 4, 8, id, 4,
                        deepest_cstate, 4, last_cstate, 4, wake_reason)
...
def context_switch_table(id, machine_id, time, cpu, thread_out_id,
                         comm_out_id, thread_in_id, comm_in_id, flags, *x):
    fmt = "!hiqiqiqiiiqiqiqiqii"
    value = struct.pack(fmt, 9, 8, id, 8, machine_id, 8, time, 4, cpu, 8,
                        thread_out_id, 8, comm_out_id, 8, thread_in_id, 8, comm_in_id, 4, flags)
