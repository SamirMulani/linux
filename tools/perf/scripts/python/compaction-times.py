...
def compaction__mm_compaction_migratepages(common_pid, nr_migrated, nr_failed):
    chead.increment_pending(common_pid,
                            pair(nr_migrated, nr_failed), None, None)


def compaction__mm_compaction_isolate_freepages(common_pid, nr_scanned, nr_taken):
    chead.increment_pending(common_pid,
                            None, pair(nr_scanned, nr_taken), None)


def compaction__mm_compaction_isolate_migratepages(common_pid, nr_scanned, nr_taken):
    chead.increment_pending(common_pid,
                            None, None, pair(nr_scanned, nr_taken))


def compaction__mm_compaction_end(common_secs, common_nsecs, common_pid):
    chead.complete_pending(common_pid, common_secs, common_nsecs)


def compaction__mm_compaction_begin(
        ...
        if opt[0] == "-":
            ...
            if m is not None and m.group() != "":
                ...
                try:
                    ...
                    sys.stderr.write(f"invalid regex {0}".format(opt))
                    sys.exit(USAGE)
