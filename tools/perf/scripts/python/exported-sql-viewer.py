...
class FindBar():
    ...
    def NotFound(self):
        QMessageBox.information(
            self.bar, "Find", "'" + self.textbox.currentText() + "' not found")
...
class CallGraphLevelTwoPlusItemBase(CallGraphLevelItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, thread_id, call_path_id, time, insn_cnt, cyc_cnt, branch_count, parent_item):
        super(CallGraphLevelTwoPlusItemBase, self).__init__(
            glb, params, row, parent_item)
    ...
    def Select(self):
        ...
        while query.next():
            ...
            child_item = CallGraphLevelThreeItem(self.glb, self.params, self.child_count, self.comm_id, self.thread_id, query.value(
                0), query.value(1), query.value(2), query.value(3), int(query.value(4)), insn_cnt, cyc_cnt, branch_count, self)
...
class CallGraphLevelThreeItem(CallGraphLevelTwoPlusItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, thread_id, call_path_id, name, dso, count, time, insn_cnt, cyc_cnt, branch_count, parent_item):
        super(CallGraphLevelThreeItem, self).__init__(glb, params, row, comm_id,
                                                      thread_id, call_path_id, time, insn_cnt, cyc_cnt, branch_count, parent_item)
        dso = dsoname(dso)
        if self.params.have_ipc:
            ...
            self.data = [name, dso, str(count), str(time), PercentToOneDP(time, parent_item.time), str(
                insn_cnt), insn_pcnt, str(cyc_cnt), cyc_pcnt, ipc, str(branch_count), br_pcnt]
        else:
            self.data = [name, dso, str(count), str(time), PercentToOneDP(time, parent_item.time), str(
                branch_count), PercentToOneDP(branch_count, parent_item.branch_count)]
...
class CallGraphLevelTwoItem(CallGraphLevelTwoPlusItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, thread_id, pid, tid, parent_item):
        super(CallGraphLevelTwoItem, self).__init__(glb, params,
                                                    row, comm_id, thread_id, 1, 0, 0, 0, 0, parent_item)
        if self.params.have_ipc:
    ...
    def Select(self):
        ...
        for child_item in self.child_items:
            ...
            if self.params.have_ipc:
                child_item.data[6] = PercentToOneDP(
                    child_item.insn_cnt, self.insn_cnt)
                child_item.data[8] = PercentToOneDP(
                    child_item.cyc_cnt, self.cyc_cnt)
                child_item.data[11] = PercentToOneDP(
                    child_item.branch_count, self.branch_count)
            else:
                child_item.data[6] = PercentToOneDP(
                    child_item.branch_count, self.branch_count)
...
class CallGraphLevelOneItem(CallGraphLevelItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, comm, parent_item):
        super(CallGraphLevelOneItem, self).__init__(
            glb, params, row, parent_item)
        if self.params.have_ipc:
            self.data = [comm, "", "", "", "", "", "", "", "", "", "", ""]
        else:
    ...
    def Select(self):
        ...
        while query.next():
            child_item = CallGraphLevelTwoItem(self.glb, self.params, self.child_count, self.dbid, query.value(
                0), query.value(1), query.value(2), self)
            self.child_items.append(child_item)
            self.child_count += 1
    ...
    def __init__(self, glb, params):
        ...
        while query.next():
            if not query.value(0):
                continue
            child_item = CallGraphLevelOneItem(
                glb, params, self.child_count, query.value(0), query.value(1), self)
            self.child_items.append(child_item)
            self.child_count += 1
...
class CallGraphModelParams():
    ...
    def __init__(self, glb, parent=None):
        self.have_ipc = IsSelectable(
            glb.db, "calls", columns="insn_count, cyc_count")
...
class CallGraphModelBase(TreeModel):
    ...
    def __init__(self, glb, parent=None):
        super(CallGraphModelBase, self).__init__(
            glb, CallGraphModelParams(glb), parent)
    ...
    def FindThread(self, c):
        if c.direction == 0 or c.value != c.last_value or c.pattern != c.last_pattern:
            ids = self.FindValue(c.value, c.pattern, c.query,
                                 c.last_value, c.last_pattern)
        elif c.direction > 0:
    ...
    def Find(self, value, direction, pattern, context, callback):
        class Context():
            ...
            def Update(self, *x):
                self.value, self.direction, self.pattern, self.last_value, self.last_pattern = x + \
                    (self.value, self.pattern)
        if len(context):
            context[0].Update(value, direction, pattern)
        else:
            context.append(Context(value, direction, pattern,
                           QSqlQuery(self.glb.db), None, None))
        # Use a thread so the UI is not blocked during the SELECT
        thread = Thread(self.FindThread, context[0])
        thread.done.connect(lambda ids, t=thread, c=callback: self.FindDone(
            t, c, ids), Qt.QueuedConnection)
        thread.start()
...
class CallGraphModel(CallGraphModelBase):
    ...
    def columnHeader(self, column):
        if self.params.have_ipc:
            headers = ["Call Path", "Object", "Count ", "Time (ns) ", "Time (%) ", "Insn Cnt",
                       "Insn Cnt (%)", "Cyc Cnt", "Cyc Cnt (%)", "IPC", "Branch Count ", "Branch Count (%) "]
        else:
            headers = ["Call Path", "Object", "Count ",
                       "Time (ns) ", "Time (%) ", "Branch Count ", "Branch Count (%) "]
        return headers[column]

    def columnAlignment(self, column):
        if self.params.have_ipc:
            alignment = [Qt.AlignLeft, Qt.AlignLeft, Qt.AlignRight, Qt.AlignRight, Qt.AlignRight, Qt.AlignRight,
...
class CallTreeLevelTwoPlusItemBase(CallGraphLevelItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, thread_id, calls_id, call_time, time, insn_cnt, cyc_cnt, branch_count, parent_item):
        super(CallTreeLevelTwoPlusItemBase, self).__init__(
            glb, params, row, parent_item)
        self.comm_id = comm_id
        self.thread_id = thread_id
    ...
    def Select(self):
        self.query_done = True
        if self.calls_id == 0:
            comm_thread = " AND comm_id = " + \
                str(self.comm_id) + " AND thread_id = " + str(self.thread_id)
        else:
            comm_thread = ""
        if self.params.have_ipc:
        ...
        while query.next():
            if self.params.have_ipc:
                ...
                cyc_cnt = 0
                branch_count = int(query.value(5))
            child_item = CallTreeLevelThreeItem(self.glb, self.params, self.child_count, self.comm_id, self.thread_id, query.value(
                0), query.value(1), query.value(2), query.value(3), int(query.value(4)), insn_cnt, cyc_cnt, branch_count, self)
            self.child_items.append(child_item)
            self.child_count += 1
...
class CallTreeLevelThreeItem(CallTreeLevelTwoPlusItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, thread_id, calls_id, name, dso, call_time, time, insn_cnt, cyc_cnt, branch_count, parent_item):
        super(CallTreeLevelThreeItem, self).__init__(glb, params, row, comm_id, thread_id,
                                                     calls_id, call_time, time, insn_cnt, cyc_cnt, branch_count, parent_item)
        dso = dsoname(dso)
        if self.params.have_ipc:
            insn_pcnt = PercentToOneDP(insn_cnt, parent_item.insn_cnt)
            cyc_pcnt = PercentToOneDP(cyc_cnt, parent_item.cyc_cnt)
            br_pcnt = PercentToOneDP(branch_count, parent_item.branch_count)
            ipc = CalcIPC(cyc_cnt, insn_cnt)
            self.data = [name, dso, str(call_time), str(time), PercentToOneDP(time, parent_item.time), str(
                insn_cnt), insn_pcnt, str(cyc_cnt), cyc_pcnt, ipc, str(branch_count), br_pcnt]
        else:
            self.data = [name, dso, str(call_time), str(time), PercentToOneDP(time, parent_item.time), str(
                branch_count), PercentToOneDP(branch_count, parent_item.branch_count)]
...
class CallTreeLevelOneItem(CallGraphLevelItemBase):
    ...
    def __init__(self, glb, params, row, comm_id, comm, parent_item):
        super(CallTreeLevelOneItem, self).__init__(
            glb, params, row, parent_item)
        if self.params.have_ipc:
            self.data = [comm, "", "", "", "", "", "", "", "", "", "", ""]
        else:
    ...
    def Select(self):
        ...
        QueryExec(query, "SELECT thread_id, pid, tid"
                  ...
                  " WHERE comm_id = " + str(self.dbid))
        while query.next():
            child_item = CallTreeLevelTwoItem(self.glb, self.params, self.child_count, self.dbid, query.value(
                0), query.value(1), query.value(2), self)
            self.child_items.append(child_item)
            self.child_count += 1
...
class CallTreeRootItem(CallGraphLevelItemBase):
    ...
    def __init__(self, glb, params):
        ...
        while query.next():
            if not query.value(0):
                continue
            child_item = CallTreeLevelOneItem(
                glb, params, self.child_count, query.value(0), query.value(1), self)
            self.child_items.append(child_item)
            self.child_count += 1
...
class CallTreeModel(CallGraphModelBase):
    ...
    def columnHeader(self, column):
        if self.params.have_ipc:
            headers = ["Call Path", "Object", "Call Time", "Time (ns) ", "Time (%) ", "Insn Cnt",
                       "Insn Cnt (%)", "Cyc Cnt", "Cyc Cnt (%)", "IPC", "Branch Count ", "Branch Count (%) "]
        else:
            headers = ["Call Path", "Object", "Call Time",
                       "Time (ns) ", "Time (%) ", "Branch Count ", "Branch Count (%) "]
        return headers[column]
...
class CallGraphWindow(TreeWindowBase):
    ...
    def __init__(self, glb, parent=None):
        ...
        self.model = LookupCreateModel(
            "Context-Sensitive Call Graph", lambda x=glb: CallGraphModel(x))
        ...
        AddSubWindow(glb.mainwindow.mdi_area, self,
...
class CallTreeWindow(TreeWindowBase):
    ...
    def __init__(self, glb, parent=None, thread_at_time=None):
        ...
        self.model = LookupCreateModel(
            "Call Tree", lambda x=glb: CallTreeModel(x))
...
class SwitchGraphDataRegion(GraphDataRegion):
    ...
    def __init__(self, key, exec_comm_id, pid, tid, comm, thread_id, comm_id):
        ...
        self.ordinal = str(pid).rjust(
            16) + str(exec_comm_id).rjust(8) + str(tid).rjust(16)
...
class GraphData(object):
    ...
    def AddPoint(self, x, y, altx=None, alty=None, hregion=None, vregion=None):
        ...
        self.points.append(GraphDataPoint(
            self, index, x, y, altx, alty, hregion, vregion))
...
class SwitchGraphData(GraphData):
        ...
        while query.next():
            ...
            if flags & 1:
                ...
                if last_thread_id == query.value(1) and last_comm_id is not None and last_comm_id != query.value(3):
                    self.SelectComms(db, last_thread_id,
                                     last_comm_id, last_time, query.value(0))
                continue
            # Schedule-in: add data point
            if len(self.points) == 0:
                start_time = self.collection.glb.StartTime(
                    self.collection.machine_id)
                hregion = self.HRegion(db, query.value(
                    1), query.value(3), start_time)
    ...
    def NewHRegion(self, db, key, thread_id, comm_id, time):
        ...
        query = QSqlQuery(db)
        QueryExec(
            query, "SELECT pid, tid FROM threads WHERE id = " + str(thread_id))
        if query.next():
            pid = query.value(0)
            tid = query.value(1)
        else:
...
class SwitchGraphDataGraphicsItem(QGraphicsItem):
    ...
    def PaintPoint(self, painter, last, x):
        if not (last is None or last.hregion.pid == 0 or x < self.attrs.subrange.x.lo):
            ...
            if width < 2:
                ...
                painter.drawLine(x0, self.graph_height -
                                 y0, x0, self.graph_height)
            else:
                painter.fillRect(x0, self.graph_height - y0,
                                 width, self.graph_height - 1, colour)
    ...
    def hoverMoveEvent(self, event):
        ...
        if time_from is not None:
            self.event_handler.PointEvent(
                self.data.cpu, time_from, time_to, hregions)
    ...
    def mousePressEvent(self, event):
        ...
        time_from, time_to, hregions, hregion_times = self.EventToData(event)
        if hregion_times:
            self.event_handler.RightClickEvent(
                self.data.cpu, hregion_times, event.screenPos())
...
class SwitchGraphGraphicsItem(QGraphicsItem):
    ...
    def __init__(self, collection, data, attrs, event_handler, first, parent=None):
        ...
        if first:
            ...
            self.scale_item.setPos(self.graph_origin_x,
                                   self.graph_origin_y + 10)

        self.yline.setPos(self.graph_origin_x - y_axis_size,
                          self.graph_origin_y - graph_height)

        self.axis_point = QGraphicsLineItem(0, 0, 0, 0, self)
        self.axis_point.setPos(self.graph_origin_x - 1,
                               self.graph_origin_y + 1)

        self.width = self.graph_origin_x + graph_width + margin
        self.height = self.graph_origin_y + margin

        self.graph = SwitchGraphDataGraphicsItem(
            data, graph_width, graph_height, attrs, event_handler, self)
        self.graph.setPos(self.graph_origin_x,
                          self.graph_origin_y - graph_height)

        if parent and 'EnableRubberBand' in dir(parent):
            parent.EnableRubberBand(
                self.graph_origin_x, self.graph_origin_x + graph_width - 1, self)
...
class VertcalGraphSetGraphicsItem(QGraphicsItem):
    ...
    def __init__(self, collection, attrs, event_handler, child_class, parent=None):
        ...
        for data in collection.data:
            child = child_class(collection, data, attrs,
                                event_handler, first, self)
    ...
    def MousePressEvent(self, event):
        ...
        if self.rubber_band is None:
            self.rubber_band = QRubberBand(
                QRubberBand.Rectangle, self.RubberBandParent())
        self.RubberBandSetGeometry(
            QRect(self.rb_origin, QSize(0, self.height)))
        if self.rb_in_view:
            self.rubber_band.show()
    ...
    def MouseReleaseEvent(self, event):
        ...
        self.rb_event_handler.RBReleaseEvent(
            self.rb_origin.x(), x, selection_state)
...
class SwitchGraphLegendModel(QAbstractTableModel):
    ...
    def __init__(self, collection, region_attributes, parent=None):
        ...
        self.child_items = sorted(
            collection.hregions.values(), key=GraphDataRegionOrdinal)
    ...
    def SetHighlight(self, row, set_highlight):
        ...
        bottom_right = self.createIndex(
            row, len(self.column_headers) - 1, child)
...
class SwitchGraphLegend(QWidget):
    ...
    def __init__(self, collection, region_attributes, parent=None):
        ...
        sz1 = self.view.columnWidth(
            0) + self.view.columnWidth(1) + self.view.columnWidth(2) + 2
...
class SwitchGraphSplitter(QSplitter):
    ...
    def resizeEvent(self, ev):
        if self.first_time:
            self.first_time = False
            sz1 = self.widget(1).view.columnWidth(
                0) + self.widget(1).view.columnWidth(1) + self.widget(1).view.columnWidth(2) + 2
            sz1 = sz1 + \
                self.widget(1).view.verticalScrollBar().sizeHint().width()
...
class SwitchGraphWidget(GraphWidget):
    ...
    def __init__(self, glb, collection, parent=None):
        ...
        xsubrange = Subrange(0.0, float(
            collection.xrangehi - collection.xrangelo) + 1.0)
        ysubrange = Subrange(0.0, float(
            collection.yrangehi - collection.yrangelo) + 1.0)
        subrange = XY(xsubrange, ysubrange)

        scale = self.GetScaleForRange(subrange)

        self.attrs = GraphAttributes(
            scale, subrange, region_attributes, collection.dp)

        self.item = VertcalGraphSetGraphicsItem(
            collection, self.attrs, self, SwitchGraphGraphicsItem)
        ...
        self.point_label.setSizePolicy(
        ...
        self.hbox = HBoxLayout(
            self.back_button, self.forward_button, self.zoom_button, self.point_label)
    ...
    def PointEvent(self, cpu, time_from, time_to, hregions):
        ...
        rel_time_from = time_from - \
            self.glb.StartTime(self.collection.machine_id)
        text = text + " Time: " + \
            str(time_from) + " (+" + ToTimeStr(rel_time_from) + ")"
    ...
    def RightClickEvent(self, cpu, hregion_times, pos):
        ...
        for hregion, time in hregion_times:
            ...
            menu_text = "Show Call Tree for {} {}:{} at {}".format(
                hregion.comm, hregion.pid, hregion.tid, time)
            menu.addAction(CreateAction(menu_text, "Show Call Tree", lambda a=None,
                           args=thread_at_time: self.RightClickSelect(args), self.view))
        menu.exec_(pos)
    ...
    def RangeEvent(self, time_from, time_to):
        ...
        text = " Time: " + str(time_from) + " (+" + ToTimeStr(rel_time_from) + \
            ") to: " + str(time_to) + " (+" + ToTimeStr(rel_time_to) + ")"
    ...
    def Title(self):
        time_from = self.collection.xrangelo + \
            Decimal(self.attrs.subrange.x.lo)
        time_to = self.collection.xrangelo + Decimal(self.attrs.subrange.x.hi)
        rel_time_from = time_from - self.start_time
        rel_time_to = time_to - self.start_time
        title = "+" + ToTimeStr(rel_time_from) + \
...
class SlowInitClass():
    ...
    def __init__(self, glb, title, init_fn):
        ...
        self.msg_box.setWindowIcon(glb.mainwindow.style(
...
class ChildDataItemFinder():
    ...
    def __init__(self, root):
        self.root = root
        self.value, self.direction, self.pattern, self.last_value, self.last_pattern = (
            None,) * 5
        self.rows = []
        self.pos = 0
    ...
    def Find(self, value, direction, pattern, context, callback):
        self.value, self.direction, self.pattern, self.last_value, self.last_pattern = (
            value, direction, pattern, self.value, self.pattern)
        # Use a thread so the UI is not blocked
        thread = Thread(self.FindThread)
        thread.done.connect(lambda row, t=thread, c=callback: self.FindDone(
            t, c, row), Qt.QueuedConnection)
        thread.start()
...
class SQLFetcher(QObject):
    ...
    def __init__(self, glb, sql, prep, process_data, parent=None):
        ...
        self.process = Process(target=SQLFetcherFn, args=(glb.dbref, sql, self.buffer, self.head, self.tail,
                               self.fetch_count, self.fetching_done, self.process_target, self.wait_event, self.fetched_event, prep))
        self.process.start()
...
class FetchMoreRecordsBar():
    ...
    def __init__(self, model, parent):
        ...
        self.label = QLabel(
            "Number of records (x " + "{:,}".format(GLB_CHUNK_SZ) + ") to fetch:")
...
class BranchLevelOneItem():
    ...
    def Select(self):
        ...
        i = 0
        while tot > 0:
            cnt, text = self.glb.disassembler.DisassembleOne(
                inst, buf_ptr, buf_sz, ip)
            if cnt:
                ...
                while k < 15:
                    byte_str += "   "
                    k += 1
                self.child_items.append(BranchLevelTwoItem(
                    0, self.br_col, byte_str + " " + text, self))
                self.child_count += 1
            else:
                return
            buf_ptr += cnt
...
class BranchModel(TreeModel):
    ...
    def __init__(self, glb, event_id, where_clause, parent=None):
        ...
        self.populated = 0
        self.have_ipc = IsSelectable(
            glb.db, "samples", columns="insn_count, cyc_count")
        if self.have_ipc:
...
class BranchWindow(QMdiSubWindow):
    ...
    def __init__(self, glb, event_id, report_vars, parent=None):
        ...
        model_name = "Branch Events " + \
            str(event_id) + " " + report_vars.UniqueId()

        self.model = LookupCreateModel(model_name, lambda: BranchModel(
            glb, event_id, report_vars.where_clause))
        ...
        self.vbox = VBox(self.view, self.find_bar.Widget(),
                         self.fetch_bar.Widget())

        self.setWidget(self.vbox.Widget())

        AddSubWindow(glb.mainwindow.mdi_area, self,
                     report_vars.name + " Branch Events")
...
class NonNegativeIntegerRangesDataItem(LineEditDataItem):
    ...
    def __init__(self, glb, label, placeholder_text, column_name, parent):
        super(NonNegativeIntegerRangesDataItem, self).__init__(
            glb, label, placeholder_text, parent)
    ...
    def DoValidate(self, input_string):
        ...
        ranges = [("(" + self.column_name + " >= " + r[0] + " AND " +
                   self.column_name + " <= " + r[1] + ")") for r in ranges]
        if len(singles):
...
class PositiveIntegerDataItem(LineEditDataItem):
    ...
    def __init__(self, glb, label, placeholder_text, parent, id="", default=""):
        super(PositiveIntegerDataItem, self).__init__(
            glb, label, placeholder_text, parent, id, default)
...
class SQLTableDataItem(LineEditDataItem):
    ...
    def __init__(self, glb, label, placeholder_text, table_name, match_column, column_name1, column_name2, parent):
        super(SQLTableDataItem, self).__init__(
            glb, label, placeholder_text, parent)
    ...
    def ValueToIds(self, value):
        ids = []
        query = QSqlQuery(self.glb.db)
        stmt = "SELECT id FROM " + self.table_name + \
            " WHERE " + self.match_column + " = '" + value + "'"
        ret = query.exec_(stmt)
        if ret:
            while query.next():
    ...
    def DoValidate(self, input_string):
        ...
        if self.column_name2:
            self.value = "( " + self.value + " OR " + \
                self.column_name2 + " IN (" + ",".join(all_ids) + ") )"
...
class SampleTimeRangesDataItem(LineEditDataItem):
    ...
    def __init__(self, glb, label, placeholder_text, column_name, parent):
        ...
        super(SampleTimeRangesDataItem, self).__init__(
            glb, label, placeholder_text, parent)

    def IdBetween(self, query, lower_id, higher_id, order):
        QueryExec(query, "SELECT id FROM samples WHERE id > " + str(lower_id) +
                  " AND id < " + str(higher_id) + " ORDER BY id " + order + " LIMIT 1")
        if query.next():
    ...
    def BinarySearchTime(self, lower_id, higher_id, target_time, get_floor):
        ...
        while True:
            ...
            QueryExec(
                query, "SELECT time FROM samples WHERE id = " + str(next_id))
            if not query.next():
    ...
    def ConvertTimeRange(self, vrange):
        ...
        vrange[1] = self.BinarySearchTime(
            1, self.last_id + 1, end_range, False)
        return True
...
class SelectedBranchDialog(ReportDialogBase):
    ...
    def __init__(self, glb, parent=None):
        ...
        items = (lambda g, p: LineEditDataItem(g, "Report name:", "Enter a name to appear in the window title bar", p, "REPORTNAME"),
                 lambda g, p: SampleTimeRangesDataItem(
                     g, "Time ranges:", "Enter time ranges", "samples.id", p),
                 lambda g, p: NonNegativeIntegerRangesDataItem(
                     g, "CPUs:", "Enter CPUs or ranges e.g. 0,5-6", "cpu", p),
                 lambda g, p: SQLTableDataItem(
                     g, "Commands:", "Only branches with these commands will be included", "comms", "comm", "comm_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "PIDs:", "Only branches with these process IDs will be included", "threads", "pid", "thread_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "TIDs:", "Only branches with these thread IDs will be included", "threads", "tid", "thread_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "DSOs:", "Only branches with these DSOs will be included", "dsos", "short_name", "samples.dso_id", "to_dso_id", p),
                 lambda g, p: SQLTableDataItem(
                     g, "Symbols:", "Only branches with these symbols will be included", "symbols", "name", "symbol_id", "to_symbol_id", p),
                 lambda g, p: LineEditDataItem(g, "Raw SQL clause: ", "Enter a raw SQL WHERE clause", p))
        super(SelectedBranchDialog, self).__init__(
            glb, title, items, True, parent)
...
def IsSelectable(db, table, sql="", columns="*"):
    query = QSqlQuery(db)
    try:
        QueryExec(query, "SELECT " + columns + " FROM " +
                  table + " " + sql + " LIMIT 1")
    except:
        return False
    return True
...
class SQLTableModel(TableModel):
    ...
    def __init__(self, glb, sql, column_headers, parent=None):
        ...
        self.fetcher = SQLFetcher(glb, sql, lambda x, y=len(
            column_headers): self.SQLTableDataPrep(x, y), self.AddSample)
        self.fetcher.done.connect(self.Update)
        self.fetcher.Fetch(GLB_CHUNK_SZ)
...
class SQLAutoTableModel(SQLTableModel):
    ...
    def __init__(self, glb, table_name, parent=None):
        ...
        if table_name == "comm_threads_view":
            ...
            sql = "SELECT * FROM " + table_name + \
                " WHERE comm_id > $$last_id$$ ORDER BY comm_id LIMIT " + \
                str(GLB_CHUNK_SZ)
        column_headers = []
        query = QSqlQuery(glb.db)
        if glb.dbref.is_sqlite3:
            ...
            QueryExec(query, "SELECT column_name FROM information_schema.columns WHERE table_schema = '" +
                      schema + "' and table_name = '" + select_table_name + "'")
            while query.next():
        ...
        super(SQLAutoTableModel, self).__init__(
            glb, sql, column_headers, parent)

    def samples_view_DataPrep(self, query, count):
        data = []
        data.append(query.value(0))
        # Workaround pyside failing to handle large integers (i.e. time) in python3 
        ...
        for col in range(min_col, max_col + 1):
            ...
            if as_csv:
                ...
                align = model.headerData(
                    col, Qt.Horizontal, Qt.TextAlignmentRole)
...
def CopyTreeCellsToClipboard(view, as_csv=False, with_hdr=False):
    ...
    if sys.version_info[0] == 3:
        ...
        expanded_mark = unicode(
            chr(0xE2) + chr(0x96) + chr(0xBC) + " ", "utf-8")
        not_expanded_mark = unicode(
            chr(0xE2) + chr(0x96) + chr(0xB6) + " ", "utf-8")
    leaf_mark = "  "
    ...
    if with_hdr:
        for c in range(col_cnt):
            ...
            if as_csv:
                ...
                width = max_width[c]
                align = model.headerData(
                    c, Qt.Horizontal, Qt.TextAlignmentRole)
                if align & Qt.AlignRight:
                    val = val.rjust(width)
                text += pad + sep + val
                pad = " " * (width - len(val))
        ...
        for c in range(col_cnt):
            ...
            if not c:
                if model.hasChildren(i):
                    ...
                    mark = leaf_mark
                val = indent_str * \
                    (i.internalPointer().level - 1) + mark + val.strip()
            if as_csv:
                text += sep + ToCSValue(val)
                sep = ","
            else:
...
class ContextMenu(object):
    ...
    def AddCopy(self, menu):
        menu.addAction(CreateAction("&Copy selection", "Copy to clipboard",
                       lambda: CopyCellsToClipboardHdr(self.view), self.view))
        menu.addAction(CreateAction("Copy selection as CS&V", "Copy to clipboard as CSV",
                       lambda: CopyCellsToClipboardCSV(self.view), self.view))
...
class TreeContextMenu(ContextMenu):
    ...
    def AddActions(self, menu):
        ...
        if len(text):
            menu.addAction(CreateAction('Copy "' + text + '"', "Copy to clipboard",
                           lambda: QApplication.clipboard().setText(text), self.view))
        self.AddCopy(menu)
...
class TableWindow(QMdiSubWindow, ResizeColumnsToContentsBase):
    ...
    def __init__(self, glb, table_name, parent=None):
        ...
        self.data_model = LookupCreateModel(
            table_name + " Table", lambda: SQLAutoTableModel(glb, table_name))
    ...
    def FindDone(self, row):
        self.find_bar.Idle()
        if row >= 0:
            self.view.setCurrentIndex(self.model.mapFromSource(
                self.data_model.index(row, 0, QModelIndex())))
        else:
            self.find_bar.NotFound()
...
class TopCallsModel(SQLTableModel):
    ...
    def __init__(self, glb, report_vars, parent=None):
        ...
        column_headers = ("Command", "PID", "TID", "Symbol", "Object", "Call Time",
                          "Return Time", "Elapsed Time (ns)", "Branch Count", "Flags")
        self.alignment = (Qt.AlignLeft, Qt.AlignLeft, Qt.AlignLeft, Qt.AlignLeft, Qt.AlignLeft,
                          Qt.AlignLeft, Qt.AlignLeft, Qt.AlignRight, Qt.AlignRight, Qt.AlignLeft)
...
class TopCallsDialog(ReportDialogBase):
    ...
    def __init__(self, glb, parent=None):
        ...
        items = (lambda g, p: LineEditDataItem(g, "Report name:", "Enter a name to appear in the window title bar", p, "REPORTNAME"),
                 lambda g, p: SQLTableDataItem(
                     g, "Commands:", "Only calls with these commands will be included", "comms", "comm", "comm_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "PIDs:", "Only calls with these process IDs will be included", "threads", "pid", "thread_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "TIDs:", "Only calls with these thread IDs will be included", "threads", "tid", "thread_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "DSOs:", "Only calls with these DSOs will be included", "dsos", "short_name", "dso_id", "", p),
                 lambda g, p: SQLTableDataItem(
                     g, "Symbols:", "Only calls with these symbols will be included", "symbols", "name", "symbol_id", "", p),
                 lambda g, p: LineEditDataItem(
                     g, "Raw SQL clause: ", "Enter a raw SQL WHERE clause", p),
...
class TopCallsWindow(QMdiSubWindow, ResizeColumnsToContentsBase):
    ...
    def __init__(self, glb, report_vars, parent=None):
        ...
        self.data_model = LookupCreateModel(
            "Top Calls " + report_vars.UniqueId(), lambda: TopCallsModel(glb, report_vars))
    ...
    def Update(self):
        ...
        for sub_window in self.mdi_area.subWindowList():
            ...
            action.triggered.connect(
                lambda a=None, x=nr: self.setActiveSubWindow(x))
...
def AddSubWindow(mdi_area, sub_window, name):
    ...
    sub_window.setWindowIcon(
        sub_window.style().standardIcon(QStyle.SP_FileIcon))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, glb, parent=None):
        ...
        edit_menu.addAction(CreateAction(
            "&Copy", "Copy to clipboard", self.CopyToClipboard, self, QKeySequence.Copy))
        edit_menu.addAction(CreateAction(
            "Copy as CS&V", "Copy to clipboard as CSV", self.CopyToClipboardCSV, self))
        edit_menu.addAction(CreateAction(
            "&Find...", "Find items", self.Find, self, QKeySequence.Find))
        edit_menu.addAction(CreateAction("Fetch &more records...", "Fetch more records",
                            self.FetchMoreRecords, self, [QKeySequence(Qt.Key_F8)]))
        edit_menu.addAction(CreateAction(
            "&Shrink Font", "Make text smaller", self.ShrinkFont, self, [QKeySequence("Ctrl+-")]))
        edit_menu.addAction(CreateAction(
            "&Enlarge Font", "Make text bigger", self.EnlargeFont, self, [QKeySequence("Ctrl++")]))

        reports_menu = menu.addMenu("&Reports")
        if IsSelectable(glb.db, "calls"):
            reports_menu.addAction(CreateAction("Context-Sensitive Call &Graph",
                                   "Create a new window containing a context-sensitive call graph", self.NewCallGraph, self))

        if IsSelectable(glb.db, "calls", "WHERE parent_id >= 0"):
            reports_menu.addAction(CreateAction(
                "Call &Tree", "Create a new window containing a call tree", self.NewCallTree, self))

        self.EventMenu(GetEventList(glb.db), reports_menu)

        if IsSelectable(glb.db, "calls"):
            reports_menu.addAction(CreateAction(
                "&Top calls by elapsed time", "Create a new window displaying top calls by elapsed time", self.NewTopCalls, self))

        if IsSelectable(glb.db, "context_switches"):
            charts_menu = menu.addMenu("&Charts")
            charts_menu.addAction(CreateAction(
                "&Time chart by CPU", "Create a new window displaying time charts by CPU", self.TimeChartByCPU, self))
        ...
        help_menu.addAction(CreateAction(
            "&About Exported SQL Viewer", "About this application", self.About, self))
    ...
    def EventMenu(self, events, reports_menu):
        ...
        for event in events:
            ...
            event = event.split(":")[0]
            if event == "branches":
                label = "All branches" if branches_events == 1 else "All branches " + \
                    "(id=" + dbid + ")"
                reports_menu.addAction(CreateAction(
                    label, "Create a new window displaying branch events", lambda a=None, x=dbid: self.NewBranchView(x), self))
                label = "Selected branches" if branches_events == 1 else "Selected branches " + \
                    "(id=" + dbid + ")"
                reports_menu.addAction(CreateAction(
                    label, "Create a new window displaying branch events", lambda a=None, x=dbid: self.NewSelectedBranchView(x), self))
    ...
    def TableMenu(self, tables, menu):
        ...
        for table in tables:
            table_menu.addAction(CreateAction(
                table, "Create a new window containing a table view", lambda a=None, t=table: self.NewTableView(t), self))
...
class Glb():
    ...
    def FileFromBuildId(self, build_id):
        file_name = self.buildid_dir + \
            build_id[0:2] + "/" + build_id[2:] + "/elf"
    ...
    def Open(self, connection_name):
        ...
        if not db.open():
            raise Exception("Failed to open database " +
                            dbname + " error: " + db.lastError().text())
        return db, dbname
